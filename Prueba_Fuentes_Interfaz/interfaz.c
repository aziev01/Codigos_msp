...
int main(void)
{
    ...
    //INICIALIZAR TIMER
    TA0CCTL0 |= CCIE;                          // CCR0 interrupt enabled FLAG1
...
//Envio msg de Encendido-Apagagado de botones 17-24
        for (h4 = 0; h4 < 8; h4++)
        {
...
//INTERRUPCION TIMER0 A0 --Rapido FLAG_1
#pragma vector=TIMER0_A0_VECTOR
__interrupt
void TIMER0_A0_ISR(void)
{
    //Detecta canto de falla si la hay activa flag de falla
    for (rb = 0; rb < 24; rb++)
    {
        if (reg_botones[rb] > 0)
        {
...
//INTERRUPCION TIMER0 A1 --Rapido FLAG_2
...
// Read interrupt vector for TA0
    ...
    case 2: // TA0CCR1
        ...
        //Revisa si algun boton tiene falla (detecta el canto), si la hay enciende el flag para que parpadee
        for (rb = 0; rb < 24; rb++)
        {
            if (reg_botones[rb] > 0)
            {
...
//INTERRUPCION TIMER 1 A0 --Lento usado para parpadear
